...
ビルド済み CLI バイナリ（Linux x86-64 `.tar.gz` / Windows x86-64 `.zip`）を [GitHub Releases](https://github.com/sumeshi/privacy-filter-jp.cpp/releases) に添付しています。ダウンロードして展開し、展開先フォルダの `pf-cli` / `pf-cli.exe` をそのまま実行できます（同梱の ggml ライブラリを隣から読み込みます）。ツールチェーンは不要です。モデル（GGUF）は下記の手順で Hugging Face から取得してください。
